# immutable-minimax-skill-supply-chain Specification

## Purpose
确保 MiniMax 插件安装的公共技能来自不可变且可验证的正式发布，并允许两个明确声明的插件专属技能安全共存。
## Requirements
...
- **THEN** 12 个受管技能的来源、提交和内容摘要均与锁文件一致
...
插件 MUST 通过声明式清单列出 `minimax-design-use` 和 `minimax-video-generation`，vendor 更新 MUST 保留它们并拒绝未声明的额外技能目录。

#### Scenario: Declared plugin-local skills exist
- **WHEN** vendor 更新公共技能
- **THEN** 两个插件专属技能保持不变并继续被三端发现
...
技能升级事件 MUST 携带 release tag 和 peeled commit SHA，插件同步流程 MUST 校验二者匹配后才可更新并创建升级 PR。

#### Scenario: Valid release event arrives
- **WHEN** `minimax-skills` 发布正式 release 并提供匹配的 tag 与 commit
- **THEN** 插件生成只包含预期技能和锁文件的可审查升级变更
...
发布前验证 MUST 确认 Codex、ZCode、Kimi manifest 和插件市场条目使用相同基础 SemVer，同时允许 Codex 构建元数据。

#### Scenario: Release candidate is consistent
- **WHEN** 发布候选通过分发检查
- **THEN** 三端基础版本一致，14 个技能边界明确，插件 tag 与 GitHub Release 可对应到同一 commit
