...
2. **白模**：Blender 白模逐镜渲染**首帧与尾帧 PNG**（占位体+固定配色， LINEAR 插值）。
   不重渲整集视频时可只用首尾帧。
3. **逐镜生成**：每镜走 FL2VA（`minimax-video-generation`）：白模首尾帧 + mmx-cli 版
   技能的风格 prompt 起草。先跑单镜 A/B 验证灰块风格泄漏，再批量。
4. **实测切点**：对拼后的成片跑 detect_shots——生成会漂移，**必须实测**，不沿用镜头表。
5. **解说后期**：video-agent-kit（timeline + 配音 + 烧录字幕 + qc）。
